...
let _lastCallTime = 0;
const MIN_GAP_MS = 4000; // 4 second gap for free tier (15 RPM = 4 seconds per request)
            ...
            for (let retry = 0; retry < 2; retry++) {
                try {
                    ...
                    if (isPermanentQuotaExhaustion) {
                        console.warn(`[Gemini] Permanent quota exhaustion detected for ${model} key=...${apiKey.slice(-6)}. Blacklisting key/model combo.`);
                        _exhaustedKeysAndModels.add(cacheKey);
                        break; // Move to next combo immediately without waiting/retrying
                    }

                    if (is429 || is5xx) {
                        // Wait before retrying this combo
                        const delay = (retry + 1) * 3000 + Math.random() * 2000;
                        console.warn(`[Gemini] ${error.status || 'error'} on ${model}. Waiting ${Math.round(delay)}ms...`);
                        await new Promise(r => setTimeout(r, delay));
                        continue; // retry same model+key
                    }

                    // Non-retryable error (400, etc.) — skip to next model
                    break;
                }
            }
        }
