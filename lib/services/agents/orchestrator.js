...
export class OrchestratorAgent {
  ...
  async planUserJourney(userData) {
    try {
      ...
      // Step 3: Get food recommendations THEN safety (sequential, not parallel)
      // This prevents rate limiting by avoiding multiple concurrent Gemini calls
      let foodRecs = null;
      let safetyInfo = null;
      
      try {
        foodRecs = await this.foodRecommendation.findFood(userData.destination || 'India', {
      ...
      try {
        safetyInfo = await this.safety.assessSafety(userData.destination || 'India');
      } catch (err) {
