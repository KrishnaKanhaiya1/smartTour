...
export class UserContextAgent {
  ...
  categorizeInterests(interests) {
    ...
    const addMatches = (rawInterest) => {
      const lower = String(rawInterest || '').toLowerCase();
      Object.entries(categories).forEach(([category, keywords]) => {
        const directMatch = lower === category || lower.includes(category) || category.includes(lower);
        const keywordMatch = keywords.some((keyword) => lower.includes(keyword) || keyword.includes(lower));
    ...
    if (interests && typeof interests === 'object') {
      Object.entries(interests).forEach(([interest, enabled]) => {
        if (!enabled) return;
        addMatches(interest);
      });
    }
  ...
  categorizeBudget(budget) {
    ...
    const budgetMap = {
      'low': { range: [0, 15000], tier: 'budget' },
      'budget': { range: [0, 15000], tier: 'budget' },
      'medium': { range: [15000, 45000], tier: 'moderate' },
      'moderate': { range: [15000, 45000], tier: 'moderate' },
      'high': { range: [45000, 80000], tier: 'premium' },
      'premium': { range: [45000, 80000], tier: 'premium' },
      'luxury': { range: [80000, Infinity], tier: 'luxury' }
    };

    if (typeof budget === 'number') {
      const val = budget < 1000 ? budget * 100 : budget; // Handle USD vs INR scales
      if (val < 15000) return budgetMap.budget;
      if (val < 45000) return budgetMap.moderate;
      if (val < 80000) return budgetMap.premium;
      return budgetMap.luxury;
    }
