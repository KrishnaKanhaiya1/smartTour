...
const DEFAULT_TTL_MS = 5 * 60 * 1000; // 5 minutes max (prevent poisoning UX with stale data)
...
class ResponseCache {
  ...
  /**
   * Generate a cache key from the prompt strings.
   */
  _makeKey(systemPrompt, userPrompt) {
    const normalized = `${systemPrompt}|||${userPrompt}`.trim();
