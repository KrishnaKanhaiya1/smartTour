  ...
  async planUserJourney(userData) {
    try {
      ...
      return {
        ...
        verifiedSources: completeJourney.verifiedSources,
        timestamp: new Date().toISOString(),
      };
    } catch (error) {
      console.error('Orchestrator Journey Error:', error);
      throw new Error(`Unable to build a verified itinerary for ${userData?.destination || 'this destination'}: ${error.message}`);
    }
  }
